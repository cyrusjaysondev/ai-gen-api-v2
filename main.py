...
async def flux_face_swap(
    ...
    watermark_image: bool = Form(False, description="Composite the GenReel logo (loaded once from /workspace/assets/genreel_logo.png) at the bottom-right. OFF by default for face-swap (the user is putting their own face into a scene — a logo would clash with the resulting artwork). Production clients (viralreel headSwapService, genaivn headSwapService) still pass true on every call, so the watermark appears in the live UI flow; this default just controls what happens when a direct-pod caller omits the field. Stacks with `watermark` if both are set."),
):
    seed = seed if seed != -1 else uuid.uuid4().int % 2**32
