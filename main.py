...
async def flux_face_swap(
    ...
    watermark_image: bool = Form(True, description="Composite the GenReel logo (loaded once from /workspace/assets/genreel_logo.png) at the bottom-right. ON by default — every face-swap output is branded. Stacks with `watermark` if both are set. Clients can pass false explicitly (admin/test); all production clients already send true."),
):
    seed = seed if seed != -1 else uuid.uuid4().int % 2**32
